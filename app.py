...
# NEW: unified multi-format ingestion engine (CSV / XLSX / PDF).
# This is the ONLY backend change. The UI below is unchanged.
...
# ---------- CSV Parsing Function (now a thin delegate to the engine) ----------
@st.cache_data
def parse_csv(uploaded_file, spending_is_negative=True):
    """Parse an uploaded file (CSV / XLSX / PDF) into the dashboard schema.
 
    Drop-in replacement for the old inline parser. Same return contract:
      * success -> (DataFrame[Date, Merchant, Category, Amount, Type], diagnostics dict)
      * failure -> (None, error message string)
    so the existing demo-data fallback below keeps working unchanged.
    """
...
with header_right:
    ...
    # type list expanded so XLSX/PDF can actually be ingested. The widget
    # itself looks identical to before.
